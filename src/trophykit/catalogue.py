...
CORE = [
    ...
    C(key="followers", title="Followers", icon="users", tok="rose", steps=[10, 50, 250, 1000, 5000], counts="Followers", src="user.followers.totalCount", word="followers"),
...
ACH = [
    ...
    A(name="Landed", g=0, icon="tick", tok="jade", rarity=1, how="Get your first pull request merged", src="search: is:pr is:merged author:you"),
    A(name="Ticket", g=0, icon="issue", tok="crimson", rarity=1, how="Open your first issue", src="user.issues.totalCount"),
    A(name="Second Opinion", g=0, icon="eye", tok="azure", rarity=1, how="Submit your first review", src="totalPullRequestReviewContributions"),
    A(name="Shipped", g=0, icon="tag", tok="teal", rarity=1, how="Publish your first release", src="repositories.releases.totalCount"),
    A(name="Shipwright", g=0, icon="package", tok="turquoise", rarity=[2, 3, 4], how="Publish 10, 50 and 200 releases", src="repositories.releases.totalCount", goals=[10, 50, 200], word="releases"),
    A(name="Merge Master", g=0, icon="check", tok="plum", rarity=[3, 4, 5], how="Get 25, 100 and 500 pull requests merged", src="search: is:pr is:merged author:you", goals=[25, 100, 500], word="merged"),
    A(name="Polyglot", g=0, icon="code", tok="amethyst", rarity=[2, 3, 4], how="Write code in 5, 10 and 20 languages", src="repositories.languages", goals=[5, 10, 20], word="languages"),
    A(name="Veteran", g=0, icon="clock", tok="brown", rarity=[1, 3, 5], how="Reach 1, 5 and 10 years on GitHub", src="user.createdAt", goals=[1, 5, 10], word="years", passive=True),
    A(name="Century", g=0, icon="chart", tok="forest", rarity=3, how="Make 100 commits in one repository", src="repository.defaultBranchRef.history(author).totalCount", goal=100, word="commits"),
    A(name="Monolith", g=0, icon="database", tok="steel", rarity=4, how="Make 1,000 commits in one repository", src="repository.defaultBranchRef.history(author).totalCount", goal=1000, word="commits"),
    A(name="Long Haul", g=0, icon="infinity", tok="ocean", rarity=4, how="Commit to a repository five years after creating it", src="repository.createdAt vs latest commit", word="repository"),
    A(name="Stargazer", g=0, icon="star", tok="amber", rarity=[3, 4, 5], how="Reach 10, 100 and 1,000 stars on one repository", src="max stargazerCount, self-stars removed", goals=[10, 100, 1000], word="stars"),
    ...
    A(name="Watched", g=0, icon="bell", tok="sky", rarity=3, how="Reach 50 watchers on one repository", src="repository.watchers.totalCount", goal=50, word="watchers"),
    ...
    A(name="By the Book", g=1, icon="book", tok="denim", rarity=[2, 3, 4], how="Write 100, 500 and 2,000 conventional commits (feat:, fix:, docs: …)", src="commit messages", goals=[100, 500, 2000], word="commits", heavy=True),
    A(name="Gitmoji", g=1, icon="sparkle", tok="honey", rarity=[2, 3], how="Start 100 and 500 commit messages with an emoji", src="commit messages", goals=[100, 500], word="commits", heavy=True),
    A(name="Signed", g=1, icon="key", tok="mustard", rarity=[2, 4], how="Sign 100 and 1,000 commits", src="commit.signature.isValid", goals=[100, 1000], word="commits", heavy=True),
    A(name="Duet", g=1, icon="users", tok="rose", rarity=2, how="Author 10 commits with a co-author", src="commit.authors.totalCount > 1", goal=10, word="commits", heavy=True),
    A(name="Undo", g=1, icon="sync", tok="clay", rarity=2, how="Revert a commit", src="commit messages starting with Revert", heavy=True),
    A(name="Surgeon", g=1, icon="target", tok="taupe", rarity=3, how="Make 50 commits that touch exactly one file", src="commit.changedFilesIfAvailable", goal=50, word="commits", heavy=True),
    ...
    A(name="Tested", g=1, icon="flask", tok="mint", rarity=2, how="Write 25 test: commits", src="commit messages", goal=25, word="commits", heavy=True),
    A(name="Scribe", g=1, icon="info", tok="lavender", rarity=2, how="Write 25 docs: commits", src="commit messages", goal=25, word="commits", heavy=True),
    A(name="Green Machine", g=1, icon="play", tok="emerald", rarity=[2, 3, 4], how="Finish 100, 1,000 and 10,000 successful workflow runs", src="REST actions/runs?status=success per repository", goals=[100, 1000, 10000], word="runs", heavy=True),
    A(name="Well Maintained", g=1, icon="download", tok="cobalt", rarity=2, how="Merge 25 Dependabot pull requests", src="search: is:pr is:merged author:app/dependabot user:you", goal=25, word="merged"),
    ...
    A(name="Weekender", g=2, icon="flag", tok="rose", rarity=2, how="Contribute on 26 weekends in one year", src="contributionCalendar", goal=26, word="weekends"),
    A(name="Night Owl", g=2, icon="moon", tok="indigo", rarity=[3, 4], how="Author 50 and 500 commits between midnight and 5 a.m.", src="commit timestamps", goals=[50, 500], word="late commits", heavy=True),
    ...
    A(name="Marathon Day", g=2, icon="bolt", tok="honey", rarity=[3, 4, 5], how="Make 25, 50 and 100 contributions in a single day", src="contributionCalendar", goals=[25, 50, 100], word="in one day"),
    ...
    A(name="Long Game", g=2, icon="layers", tok="ocean", rarity=[2, 3, 4], how="Contribute in 3, 5 and 10 different years", src="contributionsCollection per year", goals=[3, 5, 10], word="years", passive=True),
    A(name="Comeback", g=2, icon="sync", tok="jade", rarity=2, how="Contribute again after a gap of 90 days", src="contributionCalendar"),
    A(name="Friday Deploy", g=2, icon="rocket", tok="cherry", rarity=2, how="Publish a release on a Friday", src="releases.createdAt"),
    ...
    A(name="Reporter", g=3, icon="alert", tok="crimson", rarity=2, how="Open 25 issues in repositories you don’t own", src="search: is:issue author:you -user:you", goal=25, word="issues"),
    ...
    A(name="Conversation Starter", g=3, icon="mail", tok="orchid", rarity=2, how="Start 10 discussions", src="user.repositoryDiscussions.totalCount", goal=10, word="discussions"),
    A(name="Team Player", g=3, icon="layers", tok="denim", rarity=2, how="Be a public member of 3 organizations", src="user.organizations.totalCount", goal=3, word="organizations"),
    ...
    A(name="Generous", g=3, icon="plus", tok="amber", rarity=1, how="Star 100 repositories", src="user.starredRepositories.totalCount", goal=100, word="stars"),
    A(name="Curious", g=3, icon="users", tok="peach", rarity=1, how="Follow 50 people", src="user.following.totalCount", goal=50, word="follows"),
    A(name="Patron", g=3, icon="heart", tok="fuchsia", rarity=3, how="Sponsor someone on GitHub Sponsors", src="user.sponsoring.totalCount", word="sponsorship"),
    A(name="Backed", g=3, icon="medal", tok="honey", rarity=4, how="Receive a sponsorship", src="user.sponsors.totalCount", word="sponsor"),
    A(name="Gist Keeper", g=3, icon="terminal", tok="sage", rarity=1, how="Publish 10 public gists", src="user.gists.totalCount", goal=10, word="gists"),
    A(name="Planner", g=3, icon="grid", tok="lavender", rarity=2, how="Create 5 projects", src="user.projectsV2.totalCount", goal=5, word="projects"),
    ...
    A(name="Introduced", g=3, icon="pin", tok="mint", rarity=1, how="Fill in your bio, location, website and profile README", src="user.bio, location, websiteUrl, profile repository"),
    ...
    A(name="Curator", g=4, icon="tag", tok="amber", rarity=[1, 2, 3], how="Add topics to 5, 15 and 50 repositories", src="repository.repositoryTopics", goals=[5, 15, 50], word="repositories"),
    A(name="Front Door", g=4, icon="globe", tok="sky", rarity=2, how="Add a homepage link to 5 repositories", src="repository.homepageUrl", goal=5, word="repositories"),
    A(name="Welcome Mat", g=4, icon="home", tok="coral", rarity=2, how="Publish a contributing guide", src="CONTRIBUTING.md, or the account .github repository"),
    A(name="House Rules", g=4, icon="tick", tok="sage", rarity=2, how="Publish a code of conduct", src="repository.codeOfConduct"),
    A(name="Locksmith", g=4, icon="lock", tok="cherry", rarity=2, how="Publish a security policy", src="repository.isSecurityPolicyEnabled"),
    A(name="Gatekeeper", g=4, icon="server", tok="steel", rarity=3, how="Add CODEOWNERS to 5 repositories", src="object(\"HEAD:.github/CODEOWNERS\")", goal=5, word="repositories"),
    A(name="Auto-pilot", g=4, icon="gear", tok="cyan", rarity=2, how="Configure Dependabot on a repository", src="object(\"HEAD:.github/dependabot.yml\")"),
    A(name="Blueprint", g=4, icon="folder", tok="lavender", rarity=2, how="Publish a template repository", src="repository.isTemplate"),
    A(name="Open Hand", g=4, icon="heart", tok="rose", rarity=2, how="Add a FUNDING.yml", src="repository.fundingLinks"),
    A(name="Town Hall", g=4, icon="chat", tok="orchid", rarity=2, how="Turn on Discussions for a repository", src="repository.hasDiscussionsEnabled"),
    A(name="Tidy", g=4, icon="trash", tok="ash", rarity=2, how="Archive 5 repositories", src="repository.isArchived", goal=5, word="repositories"),
    A(name="Packager", g=4, icon="download", tok="plum", rarity=2, how="Attach a file to a release", src="release.releaseAssets.totalCount"),
    A(name="Semver", g=4, icon="target", tok="jade", rarity=2, how="Tag 10 releases as vMAJOR.MINOR.PATCH", src="release.tagName", goal=10, word="releases"),
    A(name="New Year", g=5, icon="sparkle", tok="mustard", rarity=2, how="Contribute on 1 January", src="contributionCalendar", secret=True),
    A(name="Leap Day", g=5, icon="calendar", tok="aqua", rarity=5, how="Contribute on 29 February", src="contributionCalendar", secret=True),
    ...
    A(name="Midnight Oil", g=5, icon="clock", tok="indigo", rarity=3, how="Author a commit at exactly midnight", src="commit timestamps", heavy=True, secret=True),
    ...
    A(name="Full House", g=5, icon="trophy", tok="honey", rarity=4, how="Hold every core trophy at Gold or better", src="core tiers", secret=True),
    ...
    A(name="Branching Out", g=0, icon="branch", tok="moss", rarity=2, how="Keep 10 branches on one repository", src="repository.refs(refPrefix: \"refs/heads/\").totalCount", goal=10, word="branches"),
    A(name="Heavy Lifter", g=1, icon="upload", tok="brick", rarity=3, how="Make a commit that changes 1,000 lines", src="commit.additions + deletions", heavy=True),
    A(name="Sweeping Change", g=1, icon="layers", tok="ruby", rarity=3, how="Make a commit that touches 50 files", src="commit.changedFilesIfAvailable", heavy=True),
    ...
    A(name="Roundtable", g=3, icon="chat", tok="mauve", rarity=2, how="Write 50 comments in Discussions", src="user.repositoryDiscussionComments.totalCount", goal=50, word="comments"),
    A(name="Form Filler", g=4, icon="info", tok="denim", rarity=2, how="Add issue templates to a repository", src="object(\"HEAD:.github/ISSUE_TEMPLATE\")"),
    A(name="Roadmap", g=4, icon="compass", tok="olive", rarity=2, how="Use 5 milestones on one repository", src="repository.milestones.totalCount", goal=5, word="milestones"),
    A(name="Label Maker", g=4, icon="tag", tok="magenta", rarity=2, how="Define 25 labels on one repository", src="repository.labels.totalCount", goal=25, word="labels"),
    A(name="Round Number", g=5, icon="target", tok="purple", rarity=3, how="Have a trophy total land exactly on 1,000", src="any core count == 1000", secret=True),
...
RACH = [
    ...
    A(name="First Release", g=0, icon="tag", tok="teal", rarity=1, how="Publish a release", src="releases.totalCount"),
    A(name="First Merge", g=0, icon="pull", tok="violet", rarity=1, how="Merge a pull request", src="pullRequests(states: MERGED)"),
    A(name="Outside Help", g=0, icon="upload", tok="cobalt", rarity=2, how="Merge a pull request from someone other than the owner", src="pullRequests(states: MERGED).author != owner"),
    ...
    A(name="First Tag", g=0, icon="pin", tok="mint", rarity=1, how="Push a tag", src="refs(refPrefix: \"refs/tags/\")"),
    A(name="Named", g=0, icon="info", tok="denim", rarity=1, how="Set a description", src="repository.description"),
    A(name="Front Door", g=0, icon="globe", tok="sky", rarity=1, how="Set a homepage link", src="repository.homepageUrl"),
    A(name="Filed", g=0, icon="tag", tok="magenta", rarity=1, how="Add 3 topics", src="repositoryTopics.totalCount", goal=3, word="topics"),
    A(name="Poster", g=0, icon="grid", tok="lavender", rarity=2, how="Upload a social preview image", src="repository.usesCustomOpenGraphImage"),
    ...
    A(name="Welcome Mat", g=1, icon="home", tok="coral", rarity=2, how="Publish a contributing guide", src="CONTRIBUTING.md, or the account .github repository"),
    A(name="House Rules", g=1, icon="tick", tok="sage", rarity=2, how="Publish a code of conduct", src="repository.codeOfConduct"),
    A(name="Locksmith", g=1, icon="lock", tok="cherry", rarity=2, how="Publish a security policy", src="repository.isSecurityPolicyEnabled"),
    A(name="Gatekeeper", g=1, icon="server", tok="steel", rarity=2, how="Add a CODEOWNERS file", src="object(\"HEAD:.github/CODEOWNERS\")"),
    A(name="Form Filler", g=1, icon="info", tok="denim", rarity=2, how="Add issue templates", src="object(\"HEAD:.github/ISSUE_TEMPLATE\")"),
    A(name="Paperwork", g=1, icon="mail", tok="mauve", rarity=2, how="Add a pull request template", src="object(\"HEAD:.github/pull_request_template.md\")"),
    A(name="Auto-pilot", g=1, icon="gear", tok="cyan", rarity=2, how="Configure Dependabot", src="object(\"HEAD:.github/dependabot.yml\")"),
    A(name="Changelog", g=1, icon="terminal", tok="olive", rarity=2, how="Keep a CHANGELOG.md", src="object(\"HEAD:CHANGELOG.md\")"),
    A(name="Well-Formed", g=1, icon="layers", tok="taupe", rarity=1, how="Add an .editorconfig", src="object(\"HEAD:.editorconfig\")"),
    A(name="Town Hall", g=1, icon="chat", tok="orchid", rarity=2, how="Turn on Discussions", src="repository.hasDiscussionsEnabled"),
    ...
    A(name="Clean Bill", g=1, icon="pulse", tok="emerald", rarity=3, how="Have zero open Dependabot alerts", src="vulnerabilityAlerts(states: OPEN), needs security-events: read"),
    A(name="Open Hand", g=1, icon="heart", tok="rose", rarity=2, how="Add a FUNDING.yml", src="repository.fundingLinks"),
    A(name="Support Line", g=1, icon="bell", tok="peach", rarity=2, how="Publish a SUPPORT.md", src="object(\"HEAD:SUPPORT.md\")"),
    A(name="Label Maker", g=1, icon="tag", tok="magenta", rarity=2, how="Define 25 labels", src="repository.labels.totalCount", goal=25, word="labels"),
    A(name="Roadmap", g=1, icon="compass", tok="olive", rarity=2, how="Use 5 milestones", src="repository.milestones.totalCount", goal=5, word="milestones"),
    A(name="By the Book", g=2, icon="book", tok="denim", rarity=[2, 3, 4], how="Land 100, 500 and 2,000 conventional commits", src="commit messages", goals=[100, 500, 2000], word="commits", heavy=True),
    A(name="Signed", g=2, icon="key", tok="mustard", rarity=[2, 4], how="Land 100 and 1,000 signed commits", src="commit.signature.isValid", goals=[100, 1000], word="commits", heavy=True),
    A(name="Gitmoji", g=2, icon="sparkle", tok="honey", rarity=[2, 3], how="Land 100 and 500 commits that start with an emoji", src="commit messages", goals=[100, 500], word="commits", heavy=True),
    A(name="Semver", g=2, icon="target", tok="jade", rarity=2, how="Tag 10 releases as vMAJOR.MINOR.PATCH", src="release.tagName", goal=10, word="releases"),
    A(name="Green Machine", g=2, icon="play", tok="emerald", rarity=[2, 3, 4], how="Finish 100, 1,000 and 10,000 successful workflow runs", src="REST actions/runs?status=success", goals=[100, 1000, 10000], word="runs"),
    ...
    A(name="Wired", g=2, icon="bolt", tok="honey", rarity=1, how="Run at least one workflow", src="object(\"HEAD:.github/workflows\")"),
    A(name="Release Notes", g=2, icon="info", tok="lavender", rarity=2, how="Write notes on 10 releases", src="release.description", goal=10, word="releases"),
    A(name="Packager", g=2, icon="download", tok="plum", rarity=2, how="Attach a file to a release", src="release.releaseAssets.totalCount"),
    A(name="Prerelease", g=2, icon="flag", tok="apricot", rarity=2, how="Publish a pre-release", src="release.isPrerelease"),
    A(name="Squeaky", g=2, icon="tick", tok="sage", rarity=3, how="Have no open pull request older than 90 days", src="pullRequests(states: OPEN).createdAt"),
    A(name="Small Steps", g=2, icon="commit", tok="moss", rarity=3, how="Keep the median commit under 50 changed lines", src="commit.additions + deletions, last 500 commits", heavy=True),
    A(name="Follows the Standards", g=2, icon="scale", tok="gold", rarity=4, how="Call tannergolden/standards from a workflow", src="workflow files containing uses: tannergolden/standards"),
    A(name="Golden Path", g=2, icon="compass", tok="gold", rarity=4, how="Be generated from tannergolden/path", src="repository.templateRepository, or the template fingerprint"),
    A(name="Containerized", g=2, icon="package", tok="ocean", rarity=2, how="Ship a Dockerfile", src="object(\"HEAD:Dockerfile\")"),
    A(name="Ready Room", g=2, icon="terminal", tok="cyan", rarity=2, how="Ship a dev container", src="object(\"HEAD:.devcontainer\")"),
    A(name="Ten Strong", g=3, icon="users", tok="rose", rarity=[2, 3, 4], how="Reach 10, 50 and 100 contributors", src="distinct contributors", goals=[10, 50, 100], word="contributors"),
    A(name="Reviewed", g=3, icon="eye", tok="azure", rarity=3, how="Have each of the last 100 merged pull requests reviewed", src="pullRequests.reviews.totalCount"),
    ...
    A(name="Good First Issues", g=3, icon="sparkle", tok="mint", rarity=2, how="Label 5 issues good first issue", src="issues(labels: [\"good first issue\"])", goal=5, word="issues"),
    A(name="Help Wanted", g=3, icon="flag", tok="coral", rarity=2, how="Label 5 issues help wanted", src="issues(labels: [\"help wanted\"])", goal=5, word="issues"),
    A(name="Outside Merges", g=3, icon="upload", tok="cobalt", rarity=[2, 3, 4], how="Merge 10, 50 and 200 pull requests from people other than the owner", src="pullRequests(states: MERGED).author != owner", goals=[10, 50, 200], word="merged"),
    A(name="Answered", g=3, icon="check", tok="sky", rarity=3, how="Mark 10 discussions as answered", src="discussions(answered: true)", goal=10, word="discussions"),
    A(name="Triage", g=3, icon="tag", tok="amber", rarity=3, how="Keep every open issue labeled", src="issues(states: OPEN) without labels == 0"),
    A(name="Well Maintained", g=3, icon="sync", tok="cobalt", rarity=2, how="Merge 25 Dependabot pull requests", src="pullRequests by app/dependabot, merged", goal=25, word="merged"),
    A(name="Talkative", g=3, icon="chat", tok="orchid", rarity=3, how="Collect 1,000 issue comments", src="issues.comments.totalCount, summed", goal=1000, word="comments"),
    A(name="Popular Opinion", g=3, icon="heart", tok="fuchsia", rarity=3, how="Have an issue with 50 thumbs up", src="issue.reactions(content: THUMBS_UP).totalCount", goal=50, word="reactions"),
    A(name="Long Thread", g=3, icon="mail", tok="mauve", rarity=2, how="Have an issue with 100 comments", src="issue.comments.totalCount", goal=100, word="comments"),
    ...
    A(name="Regulars", g=3, icon="medal", tok="honey", rarity=3, how="Have 5 contributors with 10 or more commits each", src="history authors, counted", goal=5, word="regulars"),
    ...
    A(name="Crew", g=3, icon="link", tok="steel", rarity=2, how="Have 5 collaborators", src="repository.collaborators.totalCount", goal=5, word="collaborators"),
    A(name="Watched", g=4, icon="bell", tok="sky", rarity=[2, 3, 4], how="Reach 25, 100 and 1,000 watchers", src="repository.watchers.totalCount", goals=[25, 100, 1000], word="watchers"),
    A(name="Used By", g=4, icon="layers", tok="iris", rarity=[3, 4, 5], how="Be a dependency of 10, 100 and 1,000 repositories", src="dependents, from the dependency graph", goals=[10, 100, 1000], word="dependents", heavy=True),
    A(name="Star of the Week", g=4, icon="star", tok="amber", rarity=2, how="Gain 50 stars in one week", src="ledger snapshots", goal=50, word="stars this week"),
    A(name="Trending", g=4, icon="chart", tok="tangerine", rarity=4, how="Gain 100 stars in one week", src="ledger snapshots", goal=100, word="stars this week"),
    A(name="Forked Far", g=4, icon="branch", tok="jade", rarity=3, how="Have a fork that earns 100 stars of its own", src="forks.stargazerCount", goal=100, word="stars on a fork"),
    A(name="Living Forks", g=4, icon="sync", tok="moss", rarity=3, how="Have 10 forks with commits of their own", src="forks where pushedAt > createdAt", goal=10, word="living forks"),
    ...
    A(name="Downloaded", g=4, icon="download", tok="plum", rarity=[2, 3, 4], how="Reach 1,000, 10,000 and 100,000 release downloads", src="releaseAssets.downloadCount, summed", goals=[1000, 10000, 100000], word="downloads"),
    A(name="Visited", g=4, icon="eye", tok="azure", rarity=3, how="Reach 1,000 unique visitors in two weeks", src="REST traffic/views, needs push access", goal=1000, word="visitors"),
    ...
    A(name="Referred", g=4, icon="link", tok="denim", rarity=2, how="Be linked from 5 referring sites", src="REST traffic/popular/referrers", goal=5, word="referrers"),
    ...
    A(name="Stargazer Streak", g=4, icon="sparkle", tok="honey", rarity=3, how="Gain a star in each of 12 consecutive weeks", src="ledger snapshots", goal=12, word="weeks"),
    A(name="Fork Magnet", g=4, icon="bolt", tok="lime", rarity=3, how="Gain 10 forks in one week", src="ledger snapshots", goal=10, word="forks this week"),
    ...
    A(name="Monthly Release", g=5, icon="tag", tok="teal", rarity=4, how="Publish a release in each of 12 consecutive months", src="releases.createdAt", goal=12, word="months"),
    A(name="Streak", g=5, icon="flame", tok="tangerine", rarity=[2, 3, 5], how="Commit on 30, 100 and 365 consecutive days", src="history.committedDate", goals=[30, 100, 365], word="days"),
    A(name="Comeback", g=5, icon="sync", tok="jade", rarity=2, how="Land a commit after a gap of 90 days", src="history.committedDate"),
    ...
    A(name="Weekend Project", g=5, icon="flag", tok="rose", rarity=2, how="Land commits on 26 weekends in one year", src="history.committedDate", goal=26, word="weekends"),
    ...
    A(name="Big Week", g=5, icon="chart", tok="cyan", rarity=2, how="Land 100 commits in one week", src="history.committedDate", goal=100, word="in one week"),
    ...
    A(name="Alive", g=5, icon="sun", tok="lime", rarity=1, how="Land a commit in the last 30 days", src="history.committedDate"),
    A(name="Friday Deploy", g=5, icon="rocket", tok="cherry", rarity=2, how="Publish a release on a Friday", src="releases.createdAt"),
    ...
    A(name="Round Number", g=6, icon="target", tok="purple", rarity=3, how="Have a trophy total land exactly on 1,000", src="any core count == 1000", secret=True),
    A(name="The Answer", g=6, icon="q", tok="teal", rarity=3, how="Have a trophy total read exactly 42 on a daily check", src="any core count == 42", secret=True),
    ...
    A(name="Friday the 13th", g=6, icon="alert", tok="maroon", rarity=3, how="Publish a release on a Friday the 13th", src="releases.createdAt", secret=True),
    A(name="New Year", g=6, icon="sparkle", tok="mustard", rarity=2, how="Land a commit on 1 January", src="history.committedDate", secret=True),
    ...
    A(name="Midnight Oil", g=6, icon="clock", tok="indigo", rarity=3, how="Land a commit at exactly midnight", src="commit timestamps", heavy=True, secret=True),
    ...
    A(name="Full House", g=6, icon="trophy", tok="honey", rarity=4, how="Hold every core trophy at Gold or better", src="core tiers", secret=True),
